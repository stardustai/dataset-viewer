...
#### macOS 安装
macOS 版本目前是**未签名**的，这意味着您在首次启动应用程序时可能会遇到安全警告。安装步骤：

1. **下载 `.dmg` 文件** 从发布页面
2. **打开 DMG** 并将应用拖拽到应用程序文件夹
3. **首次启动**：右键点击应用 → "打开" → "打开"（绕过 Gatekeeper）
4. **替代方法**：前往系统偏好设置 → 安全性与隐私 → 允许应用运行

> **注意**：代码签名需要 Apple 开发者计划会员资格（每年 $99）。作为开源项目，我们目前发布未签名的构建版本。应用程序完全安全可用。
